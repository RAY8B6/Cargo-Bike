...
import 'package:application_cargo/widgets/bars/search_bar_widget.dart';
...
class DeliveryMap extends StatefulWidget {
  ...
  @override
  State<DeliveryMap> createState() => _DeliveryMap();
}
...
class _DeliveryMap extends State<DeliveryMap> {
  ...
  drawMyRoad(List points) async {
    ...
    setState(() {
      infosBarre =
          "Il reste: ${roadInfo.distance?.toStringAsFixed(2)} km in ${((roadInfo.duration)! / 60).floor()}min";
    });
  }
  ...
  draw(GeoPoint start, GeoPoint end) async {
    ...
    timeTotalConvert = formattedTime(timeInSecond: timeTotal.toInt());

    infosBarre = "${distTotal.toStringAsFixed(2)} km in ${timeTotalConvert}. ";
  }
